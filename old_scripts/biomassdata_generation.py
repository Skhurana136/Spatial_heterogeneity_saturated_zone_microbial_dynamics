...
for Reg in reginvest:
    ...
    for j in Trial:
        ...
        for g in gvarnames + ["Total"]:
            if g == "Total":
                row.append([j,scdict[j]['Het'], scdict[j]['Anis'], Reg, g, summass, 1])
            else:
                row.append([j,scdict[j]['Het'], scdict[j]['Anis'], Reg, g, meanmass[gvarnames.index(g)], masscontribution[gvarnames.index(g)]])
...
cdata.to_csv(biomas_path, index=False)